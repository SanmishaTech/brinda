...
const fetchList = async (
  ...
  const response = await get(
    `/virtual-power?page=${page}&sortBy=${sortBy}&sortOrder=${sortOrder}&search=${search}&limit=${recordsPerPage}`
  );
  return response;
};

const VirtualPowerList = () => {
  ...
  const virtualPowers = data?.virtualPowers || [];
        ...
        <CardContent>
          ...
          {isLoading ? (
            ...
            <div className="text-center text-red-500">
              Failed to load Virtual Power history list.
            </div>
          ) : virtualPowers.length > 0 ? (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead
                      onClick={() => handleSort("powerPosition")}
                      className="cursor-pointer"
                    >
                      <div className="flex items-center">
                        <span>Position</span>
                        {sortBy === "powerPosition" && (
                          <span className="ml-1">
                            {sortOrder === "asc" ? (
                              <ChevronUp size={16} />
                            ) : (
                ...
                <TableBody>
                  {virtualPowers.map((list) => (
                    <TableRow key={list.id}>
                      ...
                      <TableCell className="max-w-[250px] break-words whitespace-normal">
                        {list?.powerType === "SELF" ? "SELF" : "SELF + UPLINE"}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
...
export default VirtualPowerList;
