...
const Purchase = () => {
  ...
  useEffect(() => {
    const subscription = watch((value) => {
      value.productDetails?.forEach((detail, index) => {
        ...
        if (!isNaN(qty) && !isNaN(rate)) {
          ...
          if (detail.cgstAmount !== cgstAmt) {
            setValue(`productDetails.${index}.cgstAmount`, cgstAmt);
          }
          if (detail.sgstAmount !== sgstAmt) {
            setValue(`productDetails.${index}.sgstAmount`, sgstAmt);
          }
          if (detail.igstAmount !== igstAmt) {
            setValue(`productDetails.${index}.igstAmount`, igstAmt);
          }
          if (detail.amountWithGst !== newAmountWithGst) {
            setValue(`productDetails.${index}.amountWithGst`, newAmountWithGst);
          }
        }
      });
    });
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    {memberState?.State === "Maharashtra" ? (
                      ...
                      <TableHead colSpan={2}>IGST Amount</TableHead>
                    )}
                    <TableHead>Amount With Gst</TableHead>
                    ...
                    <TableRow key={field.id}>
                      ...
                      {memberState?.State === "Maharashtra" ? (
                        <>
                          <TableCell>
                            CGST Amount: ₹
                            {watch(`productDetails.${index}.cgstAmount`) || "—"}
                            <input
                              type="hidden"
                              {...register(
                                `productDetails.${index}.cgstAmount`
                              )}
                            />
                          </TableCell>
                          <TableCell>
                            SGST Amount: ₹
                            {watch(`productDetails.${index}.sgstAmount`) || "—"}
                            <input
                              type="hidden"
                              {...register(
                                `productDetails.${index}.sgstAmount`
                              )}
                            />
                          </TableCell>
                        </>
                        ...
                        <TableCell colSpan={2}>
                          IGST Amount: ₹
                          {watch(`productDetails.${index}.igstAmount`) || "—"}
                          <input
                            type="hidden"
                            {...register(`productDetails.${index}.igstAmount`)}
                          />
                        </TableCell>
                      )}
                      ...
                      <TableCell>
                        Amount With GST: ₹
                        {watch(`productDetails.${index}.amountWithGst`) || "—"}
                        <input
                          type="hidden"
                          {...register(`productDetails.${index}.amountWithGst`)}
                        />
                      </TableCell>
