...
const fetchList = async (
  ...
  const response = await get(
    `/repurchase-payouts/list?page=${page}&sortBy=${sortBy}&sortOrder=${sortOrder}&search=${search}&limit=${recordsPerPage}`
  );
  return response;
};

const RepurchaseIncomePayoutList = () => {
  ...
  const payRepurchaseIncomeMutate = useMutation({
    mutationFn: (commissionId: number) =>
      post(`/repurchase-payouts/${commissionId}/pay`),
    onSuccess: () => {
      toast.success("Repurchase income commission paid successfully.");
    ...
    onError: (error) => {
      toast.error(error?.message || "Failed to pay repurchase income.");
    },
  });
        ...
        <CardContent>
          ...
          {isLoading ? (
            ...
            <div className="text-center text-red-500">
              Failed to load repurchase payout list.
            </div>
          ) : payoutList.length > 0 ? (
        ...
        >
          <AlertDialogHeader className="m-0 p-0">
            <AlertDialogTitle className="text-sm font-bold text-pink-600 flex items-center gap-2">
              ...
              Repurchase Income Commission Breakdown
            </AlertDialogTitle>
          ...
          <div className="grid grid-cols-2 gap-y-1 text-sm text-foreground">
            <div className="font-normal text-sm">MMI1:</div>
            <div className="text-right text-green-600 text-sm font-normal">
              {formatCurrency(selectedItem?.MMI1 ?? 0)}
            </div>

            <div className="font-normal text-sm">MMI2:</div>
            <div className="text-right text-green-600 text-sm font-normal">
              {formatCurrency(selectedItem?.MMI2 ?? 0)}
            </div>

            <div className="font-normal text-sm">Repurchase Income:</div>
            <div className="text-right text-green-600 text-sm font-normal">
              {formatCurrency(selectedItem?.repurchaseIncome ?? 0)}
            </div>

            <div className="font-normal text-sm">
              Repurchase Cashback Income:
            </div>
            <div className="text-right text-green-600 text-sm font-normal">
              {formatCurrency(selectedItem?.repurchaseCashbackIncome ?? 0)}
            </div>

            <div className="font-normal text-sm">RMI1:</div>
            <div className="text-right text-green-600 text-sm font-normal">
              {formatCurrency(selectedItem?.RMI1 ?? 0)}
            </div>

            <div className="font-normal text-sm">RMI2:</div>
            <div className="text-right text-green-600 text-sm font-normal">
              {formatCurrency(selectedItem?.RMI2 ?? 0)}
            </div>

            <div className="font-normal text-sm">RMI3:</div>
            <div className="text-right text-green-600 text-sm font-normal">
              {formatCurrency(selectedItem?.RMI3 ?? 0)}
            </div>

            {selectedItem?.TDSPercent !== "0" ? (
              <>
                <div className="font-normal text-sm">TDS %:</div>
            ...
            {selectedItem?.TDSAmount !== "0" ? (
              <>
                <div className="font-normal text-sm">TDS Amount:</div>
                <div className="text-right text-red-600 text-sm font-normal">
            ...
            <div className="font-normal text-sm">Platform Charge %:</div>
            <div className="text-right text-sm">
              {selectedItem?.platformChargePercent ?? "0.00"}%
            </div>

            <div className="font-normal text-sm">Platform Charge Amount:</div>
            <div className="text-right text-red-600 text-sm font-normal">
...
export default RepurchaseIncomePayoutList;
