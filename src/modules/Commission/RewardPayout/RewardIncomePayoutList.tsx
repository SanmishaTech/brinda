...
const fetchList = async (
  ...
  const response = await get(
    `/reward-payouts/list?page=${page}&sortBy=${sortBy}&sortOrder=${sortOrder}&search=${search}&limit=${recordsPerPage}`
  );
  return response;
};

const RewardIncomePayoutList = () => {
  ...
  const payRewardIncomeMutate = useMutation({
    mutationFn: (commissionId: number) =>
      post(`/reward-payouts/${commissionId}/pay`),
    onSuccess: () => {
      toast.success("Reward income commission paid successfully.");
    ...
    onError: (error) => {
      toast.error(error?.message || "Failed to pay reward income.");
    },
  });
  ...
  return (
    <div className="mt-2 p-4 sm:p-6">
      <h1 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">
        Reward Commission Payout Management
      </h1>
        ...
        <CardContent>
          ...
          {isLoading ? (
            ...
            <div className="text-center text-red-500">
              Failed to load reward payout list.
            </div>
          ) : payoutList.length > 0 ? (
        ...
        >
          <AlertDialogHeader className="m-0 p-0">
            <AlertDialogTitle className="text-sm font-bold text-pink-600 flex items-center gap-2">
              ...
              Reward Income Commission Breakdown
            </AlertDialogTitle>
          ...
          <div className="grid grid-cols-2 gap-y-1 text-sm text-foreground">
            <div className="font-normal text-sm">Reward Income:</div>
          ...
          <AlertDialogFooter className="flex justify-between">
            ...
            <AlertDialogAction asChild>
              <Button
                ...
                onClick={() => {
                  payRewardIncomeMutate.mutate(selectedItem?.id);
                  setIsDialogOpen(false);
                }}
                disabled={payRewardIncomeMutate.isPending}
              >
                {payRewardIncomeMutate.isPending ? (
                  <>
...
export default RewardIncomePayoutList;
